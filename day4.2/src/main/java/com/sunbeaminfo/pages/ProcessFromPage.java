...
public class ProcessFromPage extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		// above 2 steps will NOT be required : since in forward
		// WC is going to discard resp buffer.
		{
			...
			// pw.flush();//commits resp only for iinderstanding not has any partical use
			// server pull => Request dispatching tech : forward
			// 1. get RD RD si the heart of the include
			RequestDispatcher rd = request.getRequestDispatcher("result");
			// 2. include
			rd.include(request, response);
			/*
			 * Retains the PW's response buffer
			 * pauses current page execution n invoke doPost of ResultPage
			 * Included page (result page) ca also the content in the response buffer
			 * controls cmes back --original page can continue on server side console
			 */
			System.out.println("control came back....");// displayed on server side console
