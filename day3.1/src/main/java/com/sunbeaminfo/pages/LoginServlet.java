 ...
 */
// line 23 and 24 will be solved by WC
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try (PrintWriter pw = response.getWriter()) {
			...
			if (user == null) // => invalid login Client pull I : for the navigation , user will have click on
				...
				// servlet is requesting WC to return either new session object or existing session obj
//				WC checks for cookie : jSESSOINID -- if it exist tries to get HttpSession obj
				// doesn't exist --> new HttpSession will be returned
				// WC adds a cookie to the response header : JSESSIONID : aelib84274b42dv
				...
				// this is done to make the data centralized
				...
				if (user.getRole().equals("admin")) {
					...
					/*
					 * WC : DISCARDS response buffer --> sends temparary redirect response SC 302 ,
					 * location=candidate_list , Set-Cookie : JSESSIONID : fdgfdhsf5647468 Response body
					 * : EMPTY web browser checks privacy settings -- cookies blocked : cookie won't
					 * be stored. cookies are accepted : checks age (expiry) = -1 cookie gets stored
